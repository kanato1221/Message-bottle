...
struct BottleExchangeService {
    ...
    private let deleteURL = URL(string: "https://asia-northeast1-shortdiary-66f95.cloudfunctions.net/deleteSentBottle")!

    func exchange(bottle: BottleMessage, clientID: String) async throws -> BottleExchangeResult {
        let requestBody = ExchangeBottleRequest(
            localBottleID: bottle.id.uuidString,
            text: bottle.text,
        ...
        return BottleExchangeResult(
            sentBottleServerID: response.submittedBottleID,
            receivedBottle: response.deliveredBottle.receivedBottle
        )
    }

    func deleteSentBottle(_ bottle: BottleMessage) async throws {
        let requestBody = DeleteSentBottleRequest(
            bottleID: bottle.serverID,
            localBottleID: bottle.id.uuidString,
            text: bottle.text,
            driftedAt: bottle.driftedAt.map { ISO8601DateFormatter().string(from: $0) }
        )
        let _: DeleteSentBottleResponse = try await post(requestBody, to: deleteURL)
    }
...
private struct ExchangeBottleResponse: Decodable {
    var submittedBottleID: String
    var deliveredBottle: DeliveredBottleResponse
}

struct BottleExchangeResult {
    var sentBottleServerID: String
    var receivedBottle: ReceivedBottle
}

private struct DeleteSentBottleRequest: Encodable {
    var bottleID: String?
    var localBottleID: String
    var text: String
    var driftedAt: String?
...
private struct DeleteSentBottleResponse: Decodable {
    var ok: Bool
}
